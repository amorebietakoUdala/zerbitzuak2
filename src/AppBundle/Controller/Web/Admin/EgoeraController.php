...
use AppBundle\Controller\Web\Admin\EgoeraFormType;
...
*/
class EgoeraController extends Controller {
    /**
    ...
    public function showAction (Egoera $egoera){
	$this->get('logger')->debug('EgoeraController->showAction->Egoera->'.$egoera->__toDebug());
//	$this->get('logger')->debug('Showing: '.$egoera->__toDebug());
	$form = $this->createForm(EgoeraFormType::class, $egoera);
	$this->get('logger')->debug('EgoeraController->showAction->Egoera->render: admin/egoera/show.html.twig');
	...
	$em = $this->getDoctrine()->getManager();
	$egoerak = $em->getRepository('AppBundle:Egoera')->findAll();
