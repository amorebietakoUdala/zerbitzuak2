    ...
    protected function debugResponse(ResponseInterface $response)
    {
        ...
        if ($contentType == 'application/json' || strpos($contentType, '+json') !== false) {
            ...
            if ($isValidHtml) {
                ...
                foreach ($crawler->filter('h1, h2')->extract(array('-text')) as $header) {
...
//
    protected function createEskatzailea(array $data)
    {
        ...
        foreach ($data as $key => $value) {
            $accessor->setValue($eskatzailea, $key, $value);
        }

        $this->getEntityManager()->persist($eskatzailea);
    ...
    protected function createErabitzailea(array $data, $plainPassword = 'admin')
    {
        ...
        foreach ($data as $key => $value) {
            $accessor->setValue($erabiltzailea, $key, $value);
        }
        $password = $this->getService('security.password_encoder')
            ->encodePassword($erabiltzailea, $plainPassword);
        $erabiltzailea->setPassword($password);

        $this->getEntityManager()->persist($erabiltzailea);
        $this->getEntityManager()->flush();
